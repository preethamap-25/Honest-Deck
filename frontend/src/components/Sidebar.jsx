    ...
    <aside className="animate-slide-in flex flex-col w-72 h-full bg-white border-r border-blue-100 shadow-[4px_0_24px_rgba(10,128,245,0.06)] z-20 shrink-0">
      ...
      <div className="flex items-center justify-between px-4 py-4 border-b border-blue-50">
        ...
        <button
          ...
          className="p-1.5 rounded-lg hover:bg-blue-50 text-slate-400 hover:text-slate-600 transition-all"
        >
          <X size={16} />
        </button>
      </div>
      ...
      <div className="px-3 pt-3 pb-2">
        <button
          ...
          className="w-full flex items-center gap-2 px-3 py-2.5 bg-blue-500 hover:bg-blue-600 text-white rounded-xl text-sm font-semibold transition-all active:scale-[0.98] shadow-sm"
        >
          <Plus size={16} />
          New Chat
        </button>
      ...
      <div className="border-t border-blue-50 px-3 py-3">
        ...
        >
          ...
          <span className="badge bg-blue-100 text-blue-700 text-xs px-1.5 py-0.5 rounded-full font-medium">
            {user.plan}
          </span>
        </NavLink>
      </div>
...
function ChatItem({
  ...
  return (
    ...
    >
      <MessageSquare
        size={14}
        className={`mt-0.5 shrink-0 ${isActive ? "text-blue-500" : "text-slate-400"}`}
      />
      <div className="flex-1 min-w-0">
        {isRenaming ? (
          <div
          ...
          <p
            className={`text-xs truncate leading-tight ${isActive ? "text-blue-800 font-semibold" : "text-slate-700"}`}
          >
            {chat.title}
          </p>
        )}
      ...
      {!isRenaming && (
        <div className="absolute right-2 top-1.5 hidden group-hover:flex items-center gap-0.5 bg-white border border-blue-100 rounded-lg px-1 py-0.5 shadow-sm">
          ...
          <button
            onClick={onTogglePin}
            className={`p-0.5 rounded transition-colors ${chat.pinned ? "text-blue-500" : "text-slate-400 hover:text-blue-500"}`}
          >
            <Pin size={11} />
          </button>
          <button
